...
bot = commands.Bot(
    ...
    initial_channels=['earthqdota']
)


@commands.cooldown(rate=1, per=10)
@bot.command(name='song', aliases=['ыщтп','песня','gtcyz','трек','nhtr','трэк','track','music','ьгышс','музыка','vepsrf','сейчасиграет'])
async def test(ctx, status=''):
    with vk_api.VkRequestsPool(vk_session) as pool:
        status = pool.method('status.get', {"user_id": "17370128"})
    ...
    if len(status.result) > 1:
        ...
        await ctx.channel.send(f'@{ctx.author.name}, сейчас ничего не играет earthq12GUSINA')
