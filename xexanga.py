...
bot = commands.Bot(
    ...
    initial_channels=['xexanga']
)


@commands.cooldown(rate=1, per=10)
@bot.command(name='song', aliases=['пiсня','ыщтп','песня','gtcyz','трек','nhtr','трэк','track','music','ьгышс','музыка','vepsrf','сейчасиграет'])
async def test(ctx, status=''):
    with vk_api.VkRequestsPool(vk_session) as pool:
        status = pool.method('status.get', {"user_id": "500370307"})
    ...
    if len(status.result) > 1:
        ...
        await ctx.channel.send(f'@{ctx.author.name}, сейчас ничего не играет xexangGuitar')
